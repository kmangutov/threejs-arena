...
export function createVillageCluster({ seed = 1, fortified = false, scale = 1, palette: colors = {} } = {}) {
  ...
  place(group, createRockFormation({ seed: seed + 31, count: 4, scale: 0.7, palette: p }), -6.5, 4.5);
...
export function createStructureGallery({ palette: colors = {} } = {}) {
  ...
  place(group, createRoundHut({ seed: 11, scale: 0.9, palette: p }), -11.5, -3.0);
  place(group, createLonghouse({ seed: 21, scale: 0.85, palette: p }), -5.3, -3.0, 0, -0.12);
  place(group, createMarketStall({ scale: 0.9, palette: p }), 1.1, -3.0);
  place(group, createWell({ scale: 0.9, palette: p }), 5.2, -3.0);
  place(group, createWatchtower({ scale: 0.82, palette: p }), 9.3, -3.0);
  place(group, createVillageGate({ scale: 0.88, palette: p }), -8.3, 5.4);
  place(group, createCastleKeep({ scale: 0.67, palette: p }), -1.0, 5.7);
  place(group, createRockFormation({ seed: 44, count: 6, scale: 0.92, palette: p }), 6.6, 5.4);
  place(group, createCampfire({ scale: 0.8, palette: p }), 10.5, 5.2);
  place(group, createCrate({ scale: 0.9, palette: p }), 9.1, 5.0);
  place(group, createBarrel({ scale: 0.86, palette: p }), 8.2, 5.1);
  place(group, createSignpost({ scale: 0.96, palette: p }), 11.7, 3.6, 0, 0.2);
  return group;
}
